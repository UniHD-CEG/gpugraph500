...
if [ "x$G500_ENABLE_RUNTIME_SCALASCA" = "xyes" ]; then
  ...
  $scalasca $mpirun --display-map -np 16 -x LD_LIBRARY_PATH=$LD_LIBRARY_PATH -x PATH=$PATH $valgrind ../cpu_2d/g500 -s $scale_factor -C 4 -gpus 1 -qs 2 -be $codec -btr $rowthreshold -btc $columnthreshold
else
