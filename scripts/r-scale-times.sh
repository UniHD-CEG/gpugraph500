...
#
# This script will generate code to display a diagram comparing the Total execution time for different scale-factors (how do the scale)
...
#       fi
#    done
    labels_x="labels_x <- c('Total time')"
...
function generate_r_plotcode {
    ...
    matplot(mat, axes = FALSE, xlab='scale factor(#)', ylab = 'time(s)',
...
if [ $# -lt 1 ] || [ "x$1" = "x-h" ] || [ "x$1" = "x--help" ]; then
    echo "This script will generate code to display a diagram comparing the Total execution time for different scale-factors (how do the scale)"
    echo ""
