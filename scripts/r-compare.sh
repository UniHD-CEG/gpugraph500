...
function get_labels {
    ...
    default_labels=""
    for jobid in $jobids; do
        #read_variable $jobid $sf "Tasks" default_label
...
function generate_r_plotcode {
    ...
    matrix="mat <- matrix(labels_y, nrow = length(labels_y), ncol = length(labels_x), byrow = TRUE, \
...
function build {
    ...
    jobid_list="$jobids $jobid_list"
    done

    generate_r_plotcode "$result_file" "$sfs" "$jobid_list"
