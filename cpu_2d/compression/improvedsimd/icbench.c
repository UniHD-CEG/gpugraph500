...
// case P_P4D:       bitdelta32(in + 1, --n, pa, in[0], mode); vbput32(out, in[0]);
// return n == 128 ? p4dencv32(pa, n, out) : p4denc32(pa, n, out);
...
unsigned char *u32enc(unsigned *__restrict in, int n, unsigned *__restrict out)
{
    unsigned *ip;
#if 0
...
unsigned char *u32dec(unsigned *__restrict in, int n, unsigned *__restrict out)
{
    unsigned *op;
#if 0
...
unsigned char *_bitunpackx32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out           ,
                             unsigned b) { unsigned i, k = 0; for (i = 0; i < n; i++, k += b) * out++ = _bitgetx32(in, b, k); return in + PAD8(n * b); }
...
unsigned char *bitf1unpackx32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out, int start,
                              unsigned b) { int i; for (i = 0; i < n; i++) out[i] = bitgetx32(in, b, i) + start + i + 1; return in + PAD8(n * b); }
unsigned char *bitfunpackx32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out, int start,
                             unsigned b) { int i; for (i = 0; i < n; i++) out[i] = bitgetx32(in, b, i) + start; return in + PAD8(n * b); }
...
unsigned char *beenc(unsigned *__restrict in, size_t n, unsigned char *__restrict out, int id, int b)
{
    int i, xb;
    switch (id & 0x3f)
    {
    ...
    case P_PCKV:   if (b < 0) { BITSIZE32(in, n, b); *out++ = b; } return n != 128 ? bitpack32(in, n, out,
                b) : bitpackv32(in, n, out, b);
#ifdef _LIBFOR
    case P_LIBFOR: return out + for_compress_unsorted(in, out, n);
#endif

    case P_SIMDV:  if (n < 128) return vbyteenc(in, n, (unsigned *)out);
        else { if (b < 0) b = maxbits(in), *out++ = b; return simdpackwn(in, n, b, (unsigned *)out); }
...
#endif
#ifdef _BTSHUF
    case P_BTSHUF: bshuf_bitshuffle(in, out, n * 4 / 32, 32, 0);
        return out + n * 4; //bitshuffle(4, const size_t blocksize, in, out, NULL);
#endif
#endif
    ...
    case P_BLZ:
    case P_BLZ4:
    case P_BZLIB:  return out + blosc_compress(1/*clevel*/, 1/*doshuffle*/, 4/*typesize*/, n * 4, in, out,
                              n * 4 + BLOSC_MAX_OVERHEAD);
#endif
#ifdef _ZLIB
    case P_ZLIB1: case P_ZLIB2: case P_ZLIB3: case P_ZLIB4: case P_ZLIB5: case P_ZLIB6: case P_ZLIB7: case P_ZLIB8:
    case P_ZLIB9:
...
unsigned char *bedec(unsigned char *__restrict in, size_t n, unsigned *__restrict out, int id, int b)
{
    ...
    case P_OPTP4 : if (n < 128) return vbytedec(in, n, out);
        else { unsigned all_array[2048]; return (unsigned char *)detailed_p4_decode(out, (unsigned *)in, all_array); }
    // --------- bit packing -------------------------------------------
    ...
    case P_SIMDV:  if (n < 128) return vbytedec(in, n, out);
...
#endif
#ifdef _LZ4
    case P_LZ4:     in += LZ4_decompress_fast((char *)in, (char *)sbuf,
                              n * 4); //bshuf_bitunshuffle(sbuf, out, n*4/32, 32, 0);
...
unsigned char *besenc(unsigned *__restrict in, size_t n, unsigned char *__restrict out, int id, int mode)
{
    ...
    {
    //----------- copy ---------------------------------------------------------------------------------------------------------
    case P_CPY:                                                                             return u32enc(in, n,
                (unsigned *)out);
    //----------- variable byte ------------------------------------------------------------------------------------------------
    case P_VB:                                                            return mode ? vbd1enc32(in, n, out,
                -1) : vbdenc32(in, n, out, 0);
#ifndef _ZIGZAG
#ifdef _MASKEDVBYTE
    case P_MVB:                                                                             return out + vbyte_encode_delta(
                    in, n, out, 0);
#endif
#endif
    case P_VBL:       bitdelta32(in, n, pa, -mode, mode);
        return vbyteenc(pa, n, (unsigned *)out);
    ...
    case P_S16:   b = bitdelta32(in + 1, --n, pa, in[0], mode); vbput32(out, in[0]);

        if (b > 28) die("simple16 overflow.bits size>28\n");
        return vs16enc(pa, n, (unsigned *)out);
    case P_S64:   b = bitdelta32(in + 1, --n, pa, in[0], mode); vbput32(out, in[0]);

        if (b > 28) die("simple-8b overflow.bits size>28\n");
    ...
    case P_P4D:       bitdelta32(in + 1, --n, pa, in[0], mode); vbput32(out, in[0]);
        return n == 128 ? p4dencv32(pa, n, out) : p4denc32(pa, n, out);
#ifndef _ZIGZAG
    case P_P4DR:  DELTR(in, n, mode, pa);                     vbput32(out, in[0]);
        return p4denc32(pa + 1, n - 1, out);
    case P_OPTP4: b = bitdelta32(in + 1, --n, pa, in[0], mode); vbput32(out, in[0]);

        if (b > 28) die("optp4 overflow.bits size>28\n");
    ...
    case P_TRSP:  bitdelta32(in, n, (unsigned *)sbuf, -mode, mode); transpose4((unsigned char *)sbuf, n * 4, out);
        return out + n * 4;
    case P_TRSPV: bitdelta32(in, n, (unsigned *)sbuf, -mode, mode); transposev4((unsigned char *)sbuf, n * 4, out);
        return out + n * 4;
    ...
    case P_LZ4:
        {
            bitdelta32(in, n, (unsigned *)out, -mode, mode);
            transposev4((unsigned char *)out, n * 4, sbuf); //  bshuf_bitshuffle(out, sbuf, n*4/32, 32, 0);
            return out + LZ4_compress((char *)sbuf, (char *)out, n * 4);
        }
#endif
#ifdef _ZLIB
...
unsigned char *besdec(unsigned char *__restrict in, size_t n, unsigned *__restrict out, int id, int mode)
{
    unsigned b, x, v;
    switch (id & 0x3f)
    {
    ...
    case P_VG8:                              in = vintg8dec(in, n,   out);          bitundx32(out, n, -mode, mode); break;
        ...
        //------------- simple family ----------------------------------------------
#ifdef _ZIGZAG
    case P_SV:    _vbget32(in, x, *out = x); in = vsdec32(in, n - 1, out + 1);      bitundx32(out + 1, n - 1, x, mode);
        break;
#else
    case P_SV:    _vbget32(in, x, *out = x); in = vsdec32(in, n - 1, out + 1);      bitundx32(out, n, -mode, mode); break;
#endif
    case P_S16:   _vbget32(in, x, *out = x); in = vs16dec((unsigned *)in, n - 1, out + 1);
...
#ifdef _ZIGZAG
    case P_P4D:   _vbget32(in, x, *out = x); in = n == 129 ? p4ddecv32(in, n - 1, out + 1) : p4ddec32(in, n - 1, out + 1);
        bitundx32(out + 1, n - 1, x, mode); break;
#else
    ...
    case P_P4DR:  _vbget32(in, x, *out = x);
        return mode ? p4dfdecx32(in, n - 1, out + 1, x) : p4df0decx32(in, n - 1, out + 1, x);
    case P_OPTP4:
        ...
        bitundx32(out, n, -mode, mode); break;
    // ------------ bit packing ----------------------------------------
    case P_FOR:   _vbget32(in, x, *out = x); b = *in++;
        return mode ? bitf1unpack32(in, n - 1, out + 1, x, b) : bitfunpack32(in, n - 1, out + 1, x, b);
    case P_PCKR:  _vbget32(in, x, *out = x); b = *in++;
        return mode ? bitf1unpackx32(in, n - 1, out + 1, x, b) : bitfunpackx32(in, n - 1, out + 1, x, b);
#endif

    case P_PCK:   _vbget32(in, x, *out = x); b = *in++;
        return mode ? bitd1unpack32(in, n - 1, out + 1, x, b) : bitdunpack32(in, n - 1, out + 1, x, b);
...
#ifdef _TRANSFORM
    case P_ZZAG:  memcpy(out, in, n * 4); bitunzigzag32(out, n, 0); return in + n * 4;
    case P_TRSP:  untranspose4((unsigned char *)in, n * 4, (unsigned char *)out); bitundx32(out, n, -mode, mode);
        return in + n * 4;
    case P_TRSPV: untransposev4((unsigned char *)in, n * 4, (unsigned char *)out); bitundx32(out, n, -mode, mode);
        return in + n * 4;
    case P_DELTA: memcpy(out, in, n * 4); bitundx32(out, n, -mode, mode); return in + n * 4;
        ...
        //---------- delta + transpose + lz77 ----------------------
#ifdef _LZ4
    case P_LZ4: in += LZ4_decompress_fast((char *)in, (char *)sbuf, n * 4);
        untransposev4(sbuf, n * 4, (unsigned char *)out); bitundx32(out, n, -mode, mode); break;
#endif
...
void print(struct libs *libs, unsigned long long n, char *s, unsigned long long *u)
{
    ...
    for (m = 0; m < k; m++)
        if (/*libs[m].tc ||*/ libs[m].l)
        {
            ...
            if (!lb->l) lb->tc = lb->td = 0.0;
            printf("%-16s%12llu\t%5.2f\t%5.2f\t%8.2f\t%8.2f\t%s\n", prtname, lb->l, (double)lb->l * 100.0 / ((double)n * 4.0),
                   (double)lb->l * 8.0 / (double)n,
...
//-----------------------------------------------------------------------------------------------
unsigned bench(unsigned *__restrict _in, unsigned _inlen, int blksize, unsigned char *__restrict _out,
               unsigned long long outsize, char *inname, tm_t tx, unsigned *__restrict cpy, int bb, int mode)
{
    int m, id, b = bb, i;
    if (!_inlen) return 0;
    if (verb > 1) { printf(":%d,", _inlen);  fflush(stdout); }
    unsigned cn;
    ...
    for (m = 0; (id = libs[m].id) >= 0; m++)
    {
        ...
        struct libs *lb = &libs[m];
        if (verb) printf("%s,%d", libs[m].s, insize);
#ifdef _BLOSC
        if (lb->id == P_BLZ4)
        ...
        if (xcheck)
        {
            ...
            TMEND
            lb->td += (double)tm_tm /
                      tm_rm;                //printf("(%d %.2f,%2u) ", cl, (double)tm_tm, tm_rm);                     printf("(%d,%d %.2f,%.2f) ", cl, lb->l, lb->tc, lb->td);
...
void usage()
{
    ...
    fprintf(stderr,
            " -cN     N = 0,1 delta for ordered integer lists)/9,10,11=1 integer per line/19,20,21:integers separated by non digit char\n");
    ...
    fprintf(stderr,
            " -fN     N = file format: 1=text:one integer per line 2=text:integers separated by non digit char, 3=bytes\n");
...
void vstest64(int id, int rm, int rx, unsigned n)
{
    ...
    for (b = rm; b <= rx; b++)
    {
        ...
        uint64_t start = 0;
        for (i = 0; i < n; i++)
            in[i] = (/*start +=*/ RND64 & (b == 64 ? 0xffffffffffffffffull : ((1ull << b) -
                                           1))); //fprintf(stderr, ".%llx ", in[0]);
...
int main(int argc, char *argv[])
{
    ...
    for (;;)
    {
        ...
        case 'b': blksize = argtoi(optarg);
            if (blksize > BLK_SIZE) blksize = BLK_SIZE; break;
    ...
    int fno, i = 0;
    if (!tm_reps) tm_reps = tm_Reps =
                                    1;                                              //printf("range=(min=%u, max=%u)\n", rm, rx);fflush(stdout);
            ...
            if (cmd)
            {
                ...
                for (ls = libss; ls->id >= 0; ls++)
                    if (!strcasecmp(ls->s, cmd))
                    {
                        ...
                        libs[i].s      = ls->s;
                        libs[i++].size = ls->size;
                        if (verb) printf("%s/", ls->s); fflush(stdout);
                        break;
                    }
    ...
    if (argc <= optind)                                                       // No file specified at commandline
    {
        ...
        _in  = amalloc(n * 4 + OVD, 64);
        if (!_in)  die("malloc err=%u", n); in = _in + 0;
        _out = amalloc(n * 5 + OVD, 64);
        if (!_out) die("malloc err=%u", n); out = _out + 12;
        _cpy = amalloc(n * 4 + OVD, 64);
        if (!_cpy) die("malloc err=%u", n); cpy = _cpy + 3;
#else
        in  = malloc(n * 4 + OVD);
        if (!in)  die("malloc err=%u", n);
        out = malloc(n * 5 + OVD);
        if (!out) die("malloc err=%u", n);
        cpy = malloc(n * 4 + OVD);
        if (!cpy) die("malloc err=%u", n);
#endif
        char s[33];
        s[0] = 0;
        ...
        else                                                                    // Benchmark w. generated data
        {
            ...
            *in = n;
            zipfgen(in + 1, a, rm, rx, n);
            for (i = 1; i <= n; i++) xbits[bsr32(in[i])]++;
            if (mode >= 0)
            {
                stprint("delta");
                ...
                {
                    ip[v] += ip[v - 1] + mode;
                    if (ip[v] >= (1u << 31)) die("overflow generating sorted array\n");
                }
            }
            else stprint("");
...
#define LSIZE 1024
                ...
                case T_TXT:
                    {
                        while (fgets(s, LSIZE, fi))
                        {
                            ...
                            if (n >= nmax)
                            {
                                ...
                                in = realloc(in, nmax * 4 + OVD);
                                if (!in) die("malloc err=%u", nmax);
                            }
                            in[n++] = u;
                        }
                    } break;
                ...
                case T_DBL:   //Test floating pint decomposition
                    {
                        ...
                        while (fread(&c, 8, 1, fi) > 0)
                        {
                            printf("%e\n", c);
                            if (!din || n >= nmax)
                            {
                                nmax = nmax ? (nmax << 1) : (1 << 20);
                                din = realloc(din, nmax * sizeof(c) + OVD);
                                if (!din)  die("malloc err=%u", nmax);
                            }
                            din[n++] = c;
                        }
                ...
                int i;
                for (i = 0; libs[i].id >= 0; i++) slibs[i].tc += libs[i].tc, slibs[i].td += libs[i].td, slibs[i].l += libs[i].l;
                totlen += l;
                continue;
            }
            ...
            if (!fi) { fprintf(stderr, "open error '%s'", inname); perror(inname); exit(-1); }
            fseeko(fi, 0, SEEK_END); unsigned long long fisize = ftello(fi);
            fseeko(fi, 0, SEEK_SET);                       //printf("fisize=%llu\n", fisize);
            ...
            out = malloc(outsize);
            if (!out) die("malloc err=%llu", fisize);
            cpy = malloc(fisize * 4 + OVD);
            if (!cpy) die("malloc err=%llu", fisize);
            in  = malloc(fisize * 4 + 1024);
            if (!in)  die("malloc err=%llu", fisize);
            ...
            while (fread(&num, 1, 4, fi) == 4 && num)
            {
                ...
                *ip++ = num;
                if (fread(ip, 4, num, fi) != num) break;
            ...
            fclose(fi);
            totlen += bench(in, ip - in, blksize, out, outsize, inname, tx, cpy, -1,
                            mode); //    printf("N#%u,%u ", (int)(ip-in), (unsigned)(totlen/1000000));
            free(in); free(cpy); free(out);
#ifdef IC_STATS
            stprint("delta");
#endif
