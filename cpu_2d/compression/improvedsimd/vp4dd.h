...
unsigned char *p4ddv32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out,                 unsigned b,
                       unsigned bx);

unsigned char *p4ddd32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out, unsigned start, unsigned b,
                       unsigned bx);
unsigned char *p4dddv32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out, unsigned start, unsigned b,
                        unsigned bx);

unsigned char *p4dd1d32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out, unsigned start, unsigned b,
                        unsigned bx);
unsigned char *p4dd1dv32(unsigned char *__restrict in, unsigned n, unsigned *__restrict out, unsigned start, unsigned b,
                         unsigned bx);
...
{
    unsigned bi, cl, u = _bitgetx32(in, b, idx * b);
    if (unlikely(p4d->xmap[bi = idx >> 6] & (1ull << (cl = (idx & 0x3f))))) u |= _bitgetx32(p4d->ex, p4d->bx,
                (p4d->cum[bi] + popcnt64(p4d->xmap[bi] & ~((~0ull) << cl))) * p4d->bx) << b;
    return u;
}
...
{
    unsigned bi, cl, u = _bitgetx16(in, b, idx * b);
    if (unlikely(p4d->xmap[bi = idx >> 6] & (1ull << (cl = (idx & 0x3f))))) u |= _bitgetx32(p4d->ex, p4d->bx,
                (p4d->cum[bi] + popcnt64(p4d->xmap[bi] & ~((~0ull) << cl))) * p4d->bx) << b;
    return u;
}
