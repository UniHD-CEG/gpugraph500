...
DistMatrix2d<vertextyp, rowoffsettyp, WOLO, ALG, PAD>::getFoldProperties() const
{
    ...
    else
    {
        ...
        newprop.size = (ua_col_size + 1LL) * ALG;
        newprop.size = (newprop.size < row_length) ? newprop.size : row_length;
...
void DistMatrix2d<vertextyp, rowoffsettyp, WOLO, ALG, PAD>::getVertexDistributionForPred(size_t count,
        ...
        size_t *local_p) const
{
    const int64_t numAlg = globalNumberOfVertex / ALG + ((globalNumberOfVertex % ALG > 0LL) ? 1LL : 0LL);
    const int64_t c_residuum = numAlg % C;
    ...
    {
        const vertexType temporalVertex = vertex_p[i];
        if (temporalVertex / sliceSizeALG1 >= c_residuum)
        {
            owner_p[i] = (temporalVertex - residuumALG) / sliceSizeALG;
            local_p[i] = (temporalVertex - residuumALG) % sliceSizeALG;
        }
        else
        {
            owner_p[i] = temporalVertex / sliceSizeALG1;
            local_p[i] = temporalVertex % sliceSizeALG1;
        }
    }
}
