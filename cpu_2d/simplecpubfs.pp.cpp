...
{
    fq_tp_type = ((MPI_Datatype) ((void *) &( ompi_mpi_int64_t))); //Frontier Queue Transport Type
    bm_type = ((MPI_Datatype) ((void *) &( ompi_mpi_unsigned_char)));
...
{
    ((store.isLocalRow(globalstart)) ? static_cast<void> (0) : __assert_fail (#store.isLocalRow(globalstart), "simplecpubfs.cpp.tau.tmp", 83, __PRETTY_FUNCTION__));
    ((insize_max <= size) ? static_cast<void> (0) : __assert_fail (#insize_max <= size, "simplecpubfs.cpp.tau.tmp", 84, __PRETTY_FUNCTION__));
