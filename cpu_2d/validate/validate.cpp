...
static int check_value_ranges(const DistMatrix2d& store, const int64_t nglobalverts, const int64_t* const pred) {
...
static int build_bfs_depth_map(const DistMatrix2d& store, const int64_t nglobalverts, const size_t nlocalverts, const size_t maxlocalverts, const int64_t root, int64_t* const pred, int* level) {
  (void)nglobalverts;
...
#pragma omp parallel for reduction(&&:validation_passed)
      ...
      printf("dmap faild\n");
  {
     ...
     * self-loops).  */
      ...
      const std::vector<DistMatrix2d::fold_prop> rowFractions = store.getFoldProperties();
