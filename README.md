...
* Add compression to the [Predecessor List Reduction](https://github.com/UniHD-CEG/gpugraph500/blob/master/cpu_2d/globalbfs.hh#L230) phase (this uncompressed data represents the ~90% of the total data movement once row and column data is sent compressed). Currently, this process is done by (i) each node computes its Frontier Queue (FQ) vertices by aplying its FQ bitmap and transmits this to the rest using a P2P communication fashion; the same process is done with the node's Predecessor List (PL) and the node's PL bitmap, (ii) This new sequence of integers is transmitted using uncompressed [MPI_AllGatherv](https://github.com/UniHD-CEG/gpugraph500/blob/master/cpu_2d/globalbfs.hh#L1081).
As result of applying the bitmap and then transmitting, the sent data results in a random-like, non ordered sequence of integers, and thus it loses the data characteristics that enable it to achieve the reduction gained in the rows and columns transfer.
